...
- **Discord Bot Dashboard**: Public-facing bot directory where users can submit their Discord bots for review. Users with the Bot Reviewer Discord role (or owner Jinsoo) can approve/reject submissions in the admin panel. Only admins and the owner can delete bots. Approved bots appear on `/bots` page with voting system. Owner's VOID bot is pre-approved and featured. All bot application events send rich Discord embeds to a dedicated channel (`DISCORD_BOT_APPS_CHANNEL_ID`) with bot details, submitter pings, reviewer information with Discord mentions, and approval/rejection reasons. Bots are automatically deleted when the submitting user's account is deleted.
